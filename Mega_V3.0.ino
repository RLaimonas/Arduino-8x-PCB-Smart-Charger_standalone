...
typedef struct
{
  ...
  const float minStorageVoltage = 3.6;           //
  const float maxStorageVoltage = 3.7;           //
  const byte modes = 5;
} CustomSettings;
...
int minEncoderStep = 2;
int lastDir = 0;
bool rotate = false;

unsigned long timeNow;
unsigned long seconds = 1000L; // Notice the L
unsigned long chargingDeadTime = seconds * 60L;
...
void rotaryEncoder()
{
  int rotaryEncoderDirection = -1 * encoder_data(); // Check for rotation

  lastDir += rotaryEncoderDirection;
  rotate = false;

  if (abs(lastDir) >= minEncoderStep)
  {
    rotate = true;
    lastDir = 0;
  }

  if (rotaryEncoderDirection != 0 && rotate && !rotaryOverrideLock) // Rotary Encoder
  {
  ...
  else if (rotaryEncoderDirection != 0 && rotate && rotaryOverrideLock)
  {
    if (module[cycleStateLast].mode - rotaryEncoderDirection >= 0 && module[cycleStateLast].mode - rotaryEncoderDirection < settings.modes)
    {
      module[cycleStateLast].mode = module[cycleStateLast].mode - rotaryEncoderDirection;
    }
    else if (module[cycleStateLast].mode - rotaryEncoderDirection == settings.modes)
    {
      module[cycleStateLast].mode = 0;
    }
    else if (module[cycleStateLast].mode - rotaryEncoderDirection < 0)
    {
      module[cycleStateLast].mode = settings.modes - 1;
    }
    lcd.clear();
    cycleStateLCD();
  }
...
void initializeVariables(byte j)
{
  ...
  module[j].start = 0;
  module[j].testing = false;
  module[j].faulty = false;
}
...
// mode 0 None
// Mode 1 Test 3 cycles
// Mode 2 Storage
// Mode 3 Charge
// Mode 4 Discharge
// Mode 5 MilliOmh

// void cellMode(byte j)
// {
//     char lcdLine0[25];
//     char lcdLine1[25];
...
void cycleStateLCDOutput(byte j, byte LCDRow0, byte LCDRow1)
{
  ...
  if (rotaryOverride > 0 && rotaryOverrideLock == 1) // When lock on cell
  {
    // cellMode(j);
    switch (module[j].mode)
    {
    ...
    // sprintf(lcdLine3, "%-20s", "   XX");
    lcd.setCursor(0, 3);
    lcd.print(lcdLine3);
  }
  else if (rotaryOverride > 0)
  {
...
void tryChargingDead(byte j)
{
  module[j].testing = true;
  timeNow = millis();
  if (timeNow - module[j].start > chargingDeadTime)
  {
    module[j].faulty = true;
    module[j].cycleState = 7;
  }
  else
  {
    module[j].cycleState = 2;
  }
}
